...
class Model(
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: str, *model_args, **kwargs):
        ...
        try:
            ...
            if kwargs.get('local_files_only', False):
                model = cls(ModelArgs(**LOCAL_MODEL_CONFIG))
                ...
                if not os.path.exists(model_path):
                    raise FileNotFoundError(f"No model file found at {pretrained_model_name_or_path}")
                
                # Load safetensors if available
                if model_path.endswith('.safetensors'):
                    from safetensors.torch import load_file
                    print(f"Loading model from {model_path}")
                    print("Device:", kwargs.get('device', 'cpu'))
                    state_dict = load_file(model_path, device=kwargs.get('device', 'cpu'))
                else:
            ...
            # Fall back to regular Hub loading if local_files_only is False
    ...
    def __init__(self, config: ModelArgs, **kwargs):
        super().__init__()
        self.config = config
